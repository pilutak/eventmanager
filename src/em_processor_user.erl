...
-export([create_user/1, delete_user/1, modify_user/1, set_password/1, set_phonecontext/1]).
...
%%%===================================================================
create_user(Event) ->
    ?INFO_MSG("Creating user: ~p", [maps:get(user, Event)]),
    create_ims_association(Event).
    

delete_user(Event) ->
    ?INFO_MSG("Deleting user: ~p", [maps:get(user, Event)]),
...
%    em_ema_server:update_hss_phonecontext(em_utils:md5_hex(UserName), PhoneContext),
%    em_srd:set_phonecontext(UserName, PhoneContext).


%set_phonecontext(#{ user := User, phonecontext := undefined} = Event ) ->
%    CContext = em_srd:get_phonecontext(User),
%    PubId = em_srd:get_sipuri(Event),

%    case CContext of
%        "tg.gl" -> ok;
%        _ -> Event1 = maps:update(phonecontext, "tg.gl", Event),
%             Event2 = maps:update(pubid, PubId, Event1),
%             em_ema_server:update_hss_phonecontext(Event2),
%             em_srd:set_phonecontext(User, "tg.gl")
%    end;
...
%delete_ims_association(#{ phone := "NODATA" } = Event) ->
