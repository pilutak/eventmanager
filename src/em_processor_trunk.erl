...
-export([create_user/1,create_trunk_user/1, delete_user/1, modify_user/1, modify_trunk_user/1]).
...
create_trunk_user(IMSAssociation) ->
    ?INFO_MSG("Creating trunk user: ~p", [maps:get(user, IMSAssociation)]),
    ok = em_srd:create_user(IMSAssociation),
...
modify_trunk_user(IMSAssociation) ->
    ...
    case CurrentUserType of
        "user" -> 
            ...
            delete_user(IMSAssociation),
            IMSAssociation1 = maps:put(group, Group, IMSAssociation),
            create_trunk_user(IMSAssociation1),
            modify_user(IMSAssociation1);
        "trunk" -> ?INFO_MSG("Already user type trunk: ~n", []),
