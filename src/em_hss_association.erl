...
create(User, #{ type := "virtual" } = Attrs, C) ->
    logger:notice("Creating IMS PSI association ~p", [User]),     
    Req = create_psi_association(User, Attrs),
    {Result, _} = send(C, Req),
    Result;

create(User, #{ type := "pilot" } = Attrs, C) ->
    logger:notice("Creating IMS pilot association ~p", [User]),     
    Req = create_association(User, Attrs),
    {Result, _} = send(C, Req),
    Result;

create(User, #{ type := "trunk" } = Attrs, C) ->
    logger:notice("Creating IMS PSI trunk association ~p", [User]),     
    Req = create_psi_association(User, Attrs),
    {Result, _} = send(C, Req),
    Result;

create(User, #{ type := "user" } = Attrs, C) ->
    logger:notice("Creating IMS association ~p", [User]),
    Req = create_association(User, Attrs),
    {Result, _} = send(C, Req),
    Result.
    
delete(User, C) ->
    logger:notice("Deleting IMS association ~p", [User]),    
...
password(User, C) ->
    logger:notice("Updating SIP Password (autogenerated) for user ~p", [User]),    
