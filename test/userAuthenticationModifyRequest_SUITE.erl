%%%-------------------------------------------------------------------
 ...
 %%% Created : 
 %%%-------------------------------------------------------------------
 -module(userAuthenticationModifyRequest_SUITE).
...
modify_authentication(Config) ->
    User = ?config(user, Config),
    Pass = ?config(pass, Config),

    ok = em_event_server:process_event(password(User, Pass)).
...
</BroadsoftOCIReportingDocument>",[User, User, Pass]).
